...
//https://libgit2.github.com/libgit2/#HEAD/group/diff
//
//18

void diff_blob_to_buffer(MEX_DEF_INPUT){
    //1 - Directly run a diff between a blob and a buffer.
    //
    //  libgit(18,1,...
    //
    ...
    //buffer will do the reverse, with GIT_DELTA_REMOVED and blob content 
    //removed.
...
void diff_commit_as_email(MEX_DEF_INPUT){
    //4 - Create an e-mail ready patch for a commit.
    //
...
void diff_find_init_options(MEX_DEF_INPUT){
    //5 - Initializes a git_diff_find_options with default values. 
    //  Equivalent to creating an instance with GIT_DIFF_FIND_OPTIONS_INIT.
    //
    //  int git_diff_find_init_options(git_diff_find_options *opts, 
    //      unsigned int version);
}

void diff_find_similar(MEX_DEF_INPUT){
    //6 - Transform a diff marking file renames, copies, etc.
    //
    //int git_diff_find_similar(git_diff *diff, 
...
void git_diff_foreach(MEX_DEF_INPUT){
    //7 - Loop over all deltas in a diff issuing callbacks.
    //
    //  libgit(18,7,...
    //
...
void diff_format_email(MEX_DEF_INPUT){
    //8 - Create an e-mail ready patch from a diff.
    //
    //	libgit(18,8,...
    //
    //
...
void diff_format_email_init_options(MEX_DEF_INPUT){
    //9 - Initializes a git_diff_format_email_options with default values.
    //
...
void diff_free(MEX_DEF_INPUT){
    //10 - Deallocate a diff.
    //
    //void git_diff_free(git_diff *diff);
}

void diff_from_buffer(MEX_DEF_INPUT){
    //11 - Read the contents of a git patch file into a git_diff object.
    //
    //  int git_diff_from_buffer(git_diff **out, const char *content,
    //          size_t content_len);
}

void diff_get_delta(MEX_DEF_INPUT){
    //12 - Return the diff delta for an entry in the diff list
    //
    //  libgit(18,12,...
    //
...
void diff_get_stats(MEX_DEF_INPUT){
    //14 - Accumulate diff statistics for all patches.
    //
...
void diff_index_to_index(MEX_DEF_INPUT){
    //15 - Create a diff with the difference between two index objects.
    //
...
void diff_index_to_workdir(MEX_DEF_INPUT){
    //16 - Create a diff between the repository index and the 
    //     workdir directory.
    //
...
void diff_init_options(MEX_DEF_INPUT){
    //17 - Initializes a git_diff_options with default values. Equivalent 
    //  to creating an instance with GIT_DIFF_OPTIONS_INIT.
    //
...
void diff_is_sorted_icase(MEX_DEF_INPUT){
    //18 - Check if deltas are sorted case sensitively or insensitively.
    //
...
void diff_merge(MEX_DEF_INPUT){
    //19 - Merge one diff into another.
    //
    //  libgit(18,19,...
    //
...
void diff_num_deltas(MEX_DEF_INPUT){
    //20 - Query how many diff records are there in a diff.
    //
...
void diff_num_deltas_of_type(MEX_DEF_INPUT){
    //21 - Query how many diff deltas are there in a diff filtered by type.
    //
    //  size_t git_diff_num_deltas_of_type(const git_diff *diff, 
    //      git_delta_t type);
...
void diff_print(MEX_DEF_INPUT){
    //22 - Iterate over a diff generating formatted text output.
    //
